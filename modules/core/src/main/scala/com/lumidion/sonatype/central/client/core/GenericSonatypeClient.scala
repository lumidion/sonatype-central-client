...
abstract class GenericSonatypeClient(overrideEndpoint: Option[String] = None) {
  ...
  private val clientBaseUrl           = s"$host/api/v1/publisher"
  protected val clientUploadBundleUrl = s"$clientBaseUrl/upload"
  protected val clientCheckStatusUrl  = s"$clientBaseUrl/status"
  protected val clientIsArtifactPublishedUrl   = s"$clientBaseUrl/published"
  protected val clientBrowseDeploymentFilesUrl = s"$clientBaseUrl/deployments/files"
