...
import com.lumidion.sonatype.central.client.core.{CheckStatusResponse, CheckPublishedStatusResponse, DeploymentId, DeploymentName}
  ...
  handler { (req: Request) =>
    {
      for {
        ...
        isPublished = repository.isPublished(DeploymentName(s"$namespace.$name-$version"))
